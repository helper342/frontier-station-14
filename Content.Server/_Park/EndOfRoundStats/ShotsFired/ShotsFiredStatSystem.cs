...
namespace Content.Server._Park.EndOfRoundStats.ShotsFired;
...
public sealed class ShotsFiredStatSystem : EntitySystem
{
    ...
    {
        if (shotsFired == 0 && _config.GetCVar<bool>(NF14CVars.ShotsFiredDisplayNone))
            return Loc.GetString("eorstats-shotsfired-noshotsfired");

        if (shotsFired == 0 || shotsFired < _config.GetCVar<int>(NF14CVars.ShotsFiredThreshold))
            return string.Empty;
